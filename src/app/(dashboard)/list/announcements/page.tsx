...
export default function AnnouncementListPage() {
  ...
  const totalPages = Math.max(1, Math.ceil(announcementsData.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedData = announcementsData.slice(startIndex, startIndex + itemsPerPage);
      ...
      <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex w-full flex-col items-center gap-4 md:w-auto md:flex-row">
          ...
          <div className="flex items-center gap-2 self-end md:self-auto">
            ...
            {role === "student" && <FormModal table="announcement" type="create" />}
          </div>
        </div>
      </div>
