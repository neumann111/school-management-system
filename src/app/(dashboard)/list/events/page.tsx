...
export default function EventListPage() {
  ...
  const totalPages = Math.max(1, Math.ceil(eventsData.length / itemsPerPage));
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedData = eventsData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const renderRow = (item: Event) => (
    <tr key={item.id}>
      ...
      <td className="hidden text-slate-500 md:table-cell">{item.startTime}</td>
      <td className="hidden text-slate-500 md:table-cell">{item.endTime}</td>
      <td>
      ...
      <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex w-full flex-col items-center gap-4 md:w-auto md:flex-row">
          ...
          <div className="flex items-center gap-2 self-end md:self-auto">
            ...
            {(role as string) === "admin" && <FormModal table="event" type="create" />}
          </div>
        </div>
      </div>
